...
class WikibaseApi:
    ...
    """ Metodo de la clase WikibaseApi encargado de poner los valores de las propiedades relativas
    a velocidad del viento y si es marino o terrestre del tornado """
    def set_tornado_values(self, tornado_query, speed_value, land_or_marine_value):
        self.set_max_wind_speed(tornado_query, speed_value)
        self.set_land_or_marine(tornado_query, land_or_marine_value)

    def create_event(self, event_info):
        # Obtenemos el id de las propiedades
        id_property_instance_of = self.get_id_by_query('instance of', 'property')
        id_property_latitude = self.get_id_by_query('latitude', 'property')
        id_property_longitude = self.get_id_by_query('longitude', 'property')
        id_property_date_event = self.get_id_by_query('date_event', 'property')
        id_property_time_event = self.get_id_by_query('time_event', 'property')
        if "speed_wind" in event_info:
            id_property_max_wind_speed_event = self.get_id_by_query('maxWindSpeed', 'property')
